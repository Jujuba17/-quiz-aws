...
document.addEventListener('DOMContentLoaded', () => {
    // --- Seleção de Elementos do DOM (com os novos botões) ---
    ...
    // ATUALIZADO: Selecionando os dois botões
    ...
    // --- Criação do botão "Desafio Oficial" (código original mantido) ---
    ...
    openDesafioButton.style.cssText = `margin-top: 10px; background-color: #ff9900; color: white; border: none; border-radius: 5px; padding: 15px 30px; font-size: 18px; font-weight: bold; cursor: pointer; transition: all 0.3s ease; width: 100%; position: relative; overflow: hidden;`;
    openDesafioButton.addEventListener('mouseenter', (e) => { e.target.style.transform = 'scale(1.05)'; e.target.style.boxShadow = '0 4px 6px rgba(0,0,0,0.1)'; });
    openDesafioButton.addEventListener('mouseleave', (e) => { e.target.style.transform = 'scale(1)'; e.target.style.boxShadow = 'none'; });
    openDesafioButton.onclick = () => { document.getElementById('start-screen').style.display = 'none'; document.getElementById('desafio-content').style.display = 'block'; };
    ...
    provaSelect.addEventListener('change', (event) => {
        currentProva = event.target.value;
        // Não carrega aqui, espera o clique no "Iniciar"
    });

    // CORRIGIDO: Função agora é async e espera o carregamento
    async function startDesafio() {
        await loadQuestions(currentProva); // Garante que as perguntas estejam prontas

        if (questions.length === 0) return; // Não inicia se o carregamento falhar
    ...
    function loadQuestion() {
        if (currentQuestionIndex < questions.length) {
            ...
            // AJUSTADO: Mostra o botão "Enviar" e esconde o "Próxima"
            submitButton.style.display = 'inline-block';
            submitButton.disabled = true; // Começa desabilitado
    ...
    function selectAnswer(event) {
        ...
        if (!Array.isArray(questionData.correct)) { // Resposta única
        ...
        // AJUSTADO: Habilita o botão "Enviar" se PELO MENOS UMA resposta for selecionada
        submitButton.disabled = selectedAnswers.length === 0;
    }

    // NOVA FUNÇÃO: Dividindo a lógica
    function submitAnswer() {
        ...
        // A MÁGICA ACONTECE AQUI: troca a visibilidade dos botões
    ...
    function nextQuestion() {
        ...
        loadQuestion(); // Simplesmente carrega a próxima pergunta
    }
    ...
    function endDesafio() {
        ...
        const createChart = () => { if (window.Chart) { /* ... seu código do gráfico ... */ } else { setTimeout(createChart, 100); } };
        createChart();
    }
    ...
    submitButton.addEventListener('click', submitAnswer); // Chama a nova função de envio
    nextButton.addEventListener('click', nextQuestion);   // Botão de próxima pergunta chama a função de avançar
    restartButton.addEventListener('click', restartDesafio);

    // REMOVIDO: A chamada inicial foi removida para corrigir o bug
    // loadQuestions(); 
});